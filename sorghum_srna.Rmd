...
fastq <- read.table("read_lengths.tsv", sep="\t", header=FALSE)
names(fastq) <- c("cell", "rep", "count") # changing header names
...
```{r fig.width=10, fig.height=6}
setwd("~/hydrogen/sorghum")
fastq <- read.table("trimmed_read_lengths.tsv", sep="\t", header=FALSE)
names(fastq) <- c("cell", "rep", "count") # changing header names
...
bs_data <- data.frame()
for (i in 1:3) {
  tmp <- read.table(paste("output_BS-",i,"_lengths.txt",sep=""), header=FALSE)
...
ggplot(bs_data, aes(x=length, y=count, colour=rep)) +
  ...
  ggtitle("Bundle Sheath sRNA read lengths")
...
m_data <- data.frame()
for (i in 1:3) {
  tmp <- read.table(paste("output_M-",i,"_lengths.txt",sep=""), header=FALSE)
  names(tmp) <- c("length", "count")
  tmp$rep <- paste("rep",i, sep="-")
  tmp <- normalise(tmp)
  m_data <- rbind(m_data,tmp)
}

ggplot(m_data, aes(x=length, y=count, colour=rep)) +
  geom_line(size=1) +
  scale_colour_brewer(type="qual", palette=6) + 
  ggtitle("Mesophyll sRNA read lengths")
